# West Michigan "No-Website" Lead List (Expanded)
**Status:** Ready for outreach
**Total Leads:** 23 (8 precision machining + 15 multi-trade)
**Objective:** Move these shops from zero/minimal digital presence to a modern website + monthly hosting subscription.
**Call Pitch:** "I shoot professional photos of your work + build a modern website. $3k upfront for photos + build. $200/mo for hosting + updates."

---

## SECTION A: PRECISION MACHINING & TOOL SHOPS (Original List)

### Tier A1: HIGH PRIORITY — Email First

| Priority | Company Name | Contact | Email | Phone | City | Notes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **1** | **East River Machine & Tool** | Adam Furst | ermt@wmis.net | (231) 767-1701 | Muskegon | Warm lead, direct contact |
| **2** | **Emerald Tool Inc** | Tom Reidy | tom@emeraldtool.net | (231) 799-9193 | Muskegon | Professional shop, full info |
| **3** | **A & C Machine & Tool** | Charles Arnson, Sr. | acmachine.muskegon@gmail.com | (231) 759-0028 | Muskegon | Direct Gmail, easy reach |
| **4** | **H D Tool & Die** | Hai Do (President) | Phone only | (616) 395-8788 | Holland | Clean shop, strong photography wedge |

### Tier A2: MEDIUM PRIORITY — Phone Follow-up

| Priority | Company Name | Contact | Email | Phone | City | Notes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **5** | **Cooper Machine & Welding** | Jeffrey Young (President) | Phone only | (231) 773-5400 | Muskegon | Heritage shop (1920s), high trust |
| **6** | **Highland Fixture & Machine** | Dave Kooiker (Owner) | info@highlandfixture.com | (616) 298-7004 | Grand Rapids | Recently relocated, timing right |
| **7** | **Action Mold & Machining** | Michael Fassbender | Phone only | (616) 452-1580 | Grand Rapids | High-precision work, industrial park |
| **8** | **Jacox Machine Shop** | Patrick Jacox (Owner) | Phone only | (517) 663-6151 | Lansing area | Heritage Michigan name |

---

## SECTION B: EXPANDED TRADES (14 New Leads by Sector)

### B1: WELDING & METAL FABRICATION

| Priority | Company Name | Contact | Email | Phone | City | Web Status | Notes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **9** | **A & B Welding & Fabrication** | — | abweldingfabrication.com | (231) 733-2661 | Muskegon | Basic/outdated | Est. 1945, 70+ years, strong local rep |
| **10** | **Porter Steel and Welding** | — | portersteelweld.com | (231) 733-4495 | Muskegon | Basic/minimal | Serves 7-county region, on-site install |

### B2: AUTO BODY & COLLISION REPAIR

| Priority | Company Name | Contact | Email | Phone | City | Web Status | Notes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **11** | **Lee's Collision Center** | — | leescollisionmuskegon.com | (231) 737-5266 | Muskegon | Basic/minimal | 75+ years, free estimates, strong local rep |
| **12** | **Essing's Auto Body & Collision** | — | essings.com | (231) 739-3177 | Muskegon | Basic/outdated | Est. 1992, insurance + collision, weak digital |
| **13** | **Ramos and Sons Auto Body** | — | ramosautobody.com | (231) 739-7240 | Muskegon | Basic/minimal | Est. 1973, family-owned, 24/7 towing |

### B3: CONCRETE & MASONRY

| Priority | Company Name | Contact | Email | Phone | City | Web Status | Notes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **14** | **Holland Concrete** | — | concrete-holland.com | (616) 310-5117 | Holland | Basic/minimal | Concrete, masonry, polishing, tuckpointing |
| **15** | **Specialty Concrete Construction LLC** | — | None verified | (616) 994-8334 | Holland | **No website** | 15 years in business, diverse services |
| **16** | **Quantum Concrete** | — | quantumconcrete.com (expired cert) | (616) 530-2444 | Grandville | **Broken/Outdated** | Licensed contractor, serving W. Michigan since 1996. Mobile-unfriendly, poor UX. HIGH PRIORITY — established customer base, obvious redesign pain. |

### B4: CONSTRUCTION & REMODELING

| Priority | Company Name | Contact | Email | Phone | City | Web Status | Notes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **17** | **Nelson Builders** | Josh Nelson | nelsonsbuild.com | (616) 335-9840 | Holland | Good but no portfolio photos | Est. 2001, award-winning, kitchen/bath specialist |

### B5: EXCAVATION & SITE WORK

| Priority | Company Name | Contact | Email | Phone | City | Web Status | Notes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **18** | **Epic Excavating** | — | epicexcavating.net | (616) 813-9596 | Grand Rapids/Hudsonville | Functional, some photos | Founded 2015, commercial + residential |
| **19** | **Cutting Edge Excavating** | — | ceexcavating.net | (616) 662-9653 | Hudsonville | Functional | Family-owned (Nykamp), 35+ yrs combined exp |

### B6: ROOFING

| Priority | Company Name | Contact | Email | Phone | City | Web Status | Notes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **20** | **Free Spirit Construction** | — | freespiritconstruction.com | (231) 739-2124 | Muskegon | Basic | Family-owned since 2008, serves W. Michigan |
| **21** | **Sherriff Goslin Roofing** | — | Verified but legacy | (231) 798-7772 | Muskegon/Lakeshore | Established brand | #1 roofing in Muskegon, est. 1925, 100+ yrs |

### B7: HVAC & PLUMBING

| Priority | Company Name | Contact | Email | Phone | City | Web Status | Notes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **22** | **TMC Mechanical** | — | tmcmechanicalllc.com | (231) [verify] | Muskegon | Basic/minimal photos | Furnace, AC, heat pump service/repair |
| **23** | **Lascko Services** | — | lasckoservices.com | [verify] | Muskegon | Functional, 4.9 rating | Family-owned, 20+ yrs, plumbing/HVAC/drain |

---

## QUICK STATS

| Metric | Count | Notes |
| :--- | :--- | :--- |
| **Total Leads** | 23 | 8 machining + 15 multi-trade |
| **Geographic Spread** | Muskegon (10), Holland (4), Grand Rapids/Hudsonville (5), Lansing (1) | West Michigan focus, lower peninsula |
| **Trades Represented** | 7 | Machining, Welding, Auto Body, Concrete, Construction, Excavation, Roofing, HVAC/Plumbing |
| **Tier 1 (Email)** | 8 | Start with these (Section A tier 1 + Section B with verified emails) |
| **Tier 2 (Phone)** | 14 | Follow-up round (all Section B + A tier 2 phone-only) |
| **Web Status: No Website** | 1 | Specialty Concrete (immediate win candidate) |
| **Web Status: Basic/Outdated** | 12 | Most need modern redesign + professional photos |
| **Web Status: Good foundation** | 9 | Have sites but need better portfolio photography |

---

## RECOMMENDED OUTREACH SEQUENCE

### PHASE 1 (Week 1–2): Machining Tier 1 + Best Multi-Trade Leads
**Goal:** 3–4 exploratory calls booked

1. **Email (Day 1–2):**
   - East River Machine
   - Emerald Tool
   - A&C Machine
   - A & B Welding (phone if no email response)

2. **Phone (Day 3–5):**
   - H D Tool (Holland — Hai Do)
   - Lee's Collision (strongest case: 75 yrs, no modern web)
   - Holland Concrete (no website = immediate win candidate)

### PHASE 2 (Week 3–4): Secondary Machining + Multi-Trade Follow-up
**Goal:** Test the model with one warm lead

3. **Phone Calls:**
   - Specialty Concrete (no website — high conversion potential)
   - Cooper Machine & Welding
   - Ramos Auto Body (family business, strong local rep)
   - Free Spirit Construction

### PHASE 3 (Week 5+): Tier 2 Pipeline
**Goal:** Build pipeline of 10–15 qualified prospects

4. **Continue with remaining leads** based on response rates from Phase 1–2

---

## OUTREACH NOTES

**Highest-Probability Targets (prioritize first):**
1. **Quantum Concrete** — broken/outdated site (expired SSL cert), 30-year established business, clear mobile UX pain. **FIRST CALL TARGET.**
2. **Specialty Concrete Construction** — no website (immediate pain point)
3. **Lee's Collision** — 75 years in business, zero online presence
4. **East River Machine** — warm lead with email
5. **Holland Concrete** — basic site, clear upgrade path

**Pattern Observed:**
- Established businesses (20–100+ years) with strong local reputation
- Limited web investment = no lead generation online
- Service businesses benefit most from professional photography
- All are operationally successful → can afford $200/mo recurring
